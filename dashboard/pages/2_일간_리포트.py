"""
DC-Pickaxe Analytics — 일간 이슈 리포트 v9
IA 개편:
  - 브레드크럼: 홈 › 리포트 목록 › 날짜
  - 이전/다음 날짜 이동 버튼 (prev/next day nav)
...
with bc4:
    st.caption('›')
with bc5:
    st.caption(f'**{report_date}** 일간 이슈 리포트' if report_date else '일간 이슈 리포트')
...
with nav_c:
    st.markdown(
        ...
        f'padding-top:6px;">{report_date}</div>',
...
with tab_report:
    ...
    with k1:
        st.metric(
            ...
            delta_color='normal',
            help=f'{report_date} 00:00~23:59 전체 합산 / 7일 일평균 {daily_avg:.1f}건 대비',
        )
    with k2:
        st.metric('7일 신규', f'{total_7d:,}건',
                  help=f'{report_date} 기준 7일 이내 전체 합산')
    with k3:
