...
from dashboard.dash_styles import gallery_card, hero_card
...
def render_cross_summary(cross_df: pd.DataFrame, date_str: str):
    ...
    st.markdown(hero_card(summary.replace('\n', '<br>')), unsafe_allow_html=True)
    if run_id:
...
def _render_single_card(result: dict):
    ...
    # 요약의 첫 번째 유의미한 줄을 미리보기로 사용
    preview = ''
    for ln in ai_summary.split('\n'):
        ln = ln.strip().lstrip('#').lstrip('*').strip()
        if len(ln) > 20:
            preview = ln[:85] + ('…' if len(ln) > 85 else '')
            break

    st.markdown(gallery_card(name, new_today, new_7d, preview), unsafe_allow_html=True)
...
def render_genre_keywords(results_df: pd.DataFrame):
    ...
    render_keyword_tags(combined.most_common(25))
