...
def summarize_weekly_gallery(
    ...
    """
    Returns: {
        headline, summary, temperature_tag, top_cause,
        category_scores, major_issues, sentiment
    }
    """
    kw_str = ", ".join(kw for kw, _ in keywords[:10]) if keywords else "없음"
    ...
    samples_md = "\n\n".join(
        f"[{i+1}] 제목: {p['제목']}\n"
        f"     댓글: {p.get('댓글수', 0)}  추천: {p.get('추천수', 0)}\n"
        f"     본문: {str(p.get('본문', '') or '')[:200] or '(없음)'}"
        for i, p in enumerate(ai_samples[:15])
    )
    tops_md = "\n".join(
        f"{i+1}. {p['제목']} — 댓글 {p.get('댓글수', 0)}, 링크: {p.get('링크', '')}"
        for i, p in enumerate(top_posts[:5])
    )
...
## 분석 대상
갤러리: {gallery_name} (기간: {week_start} ~ {week_end})
...
{{
  "headline": "이번 주 이 갤러리를 한 줄로 표현 (20자 이내)",
  "summary": "3~4문장 주간 상황 서술",
  "temperature_tag": "이번 주 분위기 한 단어",
  "top_cause": "balance | operation | bug | payment | content | 기타 중 하나",
  "category_scores": {{
    "balance":   {{"score": 0, "summary": ""}},
    "operation": {{"score": 0, "summary": ""}},
    "bug":       {{"score": 0, "summary": ""}},
...
headline (20자 이내): 이번 주 이 갤러리에서 일어난 일을 뉴스 헤드라인처럼 압축

summary (3~4문장):
- 핵심 화제/이슈 → 유저 반응 → 전반 분위기 순서로 서술
- 종결어미 '~습니다/ㅂ니다' 체 통일. 마크다운 금지. 행동 권고 금지

top_cause: 이번 주 가장 지배적인 이슈 유형
- balance: 밸런스·스펙·직업·메타
- operation: 서버·공지·CS·운영 정책
- bug: 버그·오류·기술적 결함
- payment: 결제·과금·확률
- content: 신규 컨텐츠·패치·업데이트
- 기타: 커뮤니티 자체 이슈

category_scores (각 카테고리):
- score: 0~3점 (0=해당 없음, 1=언급 있음, 2=주요 화제, 3=핵심 이슈)
- summary: 해당 카테고리에서 논의된 내용 1문장. score=0이면 빈 문자열

major_issues (최대 3개, 이번 주 주요 이슈):
- title: 이슈 제목 (15자 이내)
- detail: 구체적 설명 2~3문장
- mention_count: 해당 이슈를 다루는 게시글 수 (샘플 내 추정)
...
- negative: 부정적 반응이 있었다면 1~2문장 요약"""
    ...
    fallback: dict = {
        "headline": "", "summary": "", "temperature_tag": "", "top_cause": "기타",
        "category_scores": {
            "balance":   {"score": 0, "summary": ""},
            "operation": {"score": 0, "summary": ""},
            "bug":       {"score": 0, "summary": ""},
    ...
    return {
        ...
        "summary":         str(result.get("summary", "")),
        "temperature_tag": str(result.get("temperature_tag", "")),
        "top_cause":       str(result.get("top_cause", "기타")),
        "category_scores": category_scores,
        "major_issues":    major_issues,
        "sentiment": {
            "positive": str(sentiment.get("positive", "")),
            "negative": str(sentiment.get("negative", "")),
        },
    }
...
def summarize_monthly_gallery(
    ...
    keywords: list[list],
    headlines: list[str],
    category_scores_agg: dict | None = None,
    major_issues_sample: list[dict] | None = None,
) -> dict:
    """
    Returns: {
        headline, summary, temperature_tag,
        category_scores, major_issues, sentiment
    }
    """
    kw_str = ", ".join(kw for kw, _ in keywords[:10]) if keywords else "없음"
    ...
    # 집계된 일간 카테고리 점수 요약
    cat_context = ""
    if category_scores_agg:
        lines = []
        for k, v in category_scores_agg.items():
            if isinstance(v, dict) and v.get("max_score", 0) > 0:
                lines.append(f"  - {cause_map.get(k, k)}: 최고 {v['max_score']}점, {v['issue_days']}일 발생")
        if lines:
            cat_context = "카테고리별 발생 현황:\n" + "\n".join(lines)

    # 주요 이슈 샘플
    issues_context = ""
    if major_issues_sample:
        issues_context = "주요 이슈 샘플:\n" + "\n".join(
            f"  - {i.get('title', '')}: {i.get('detail', '')[:100]}"
            for i in major_issues_sample[:5]
        )

    prompt = f"""당신은 DC인사이드 게임 갤러리 커뮤니티 월간 동향 분석 전문가입니다.
...
{{
  "headline": "이 갤러리의 이번 달을 한 줄로 표현 (20자 이내)",
  "summary": "3~4문장 월간 동향 서술",
  "temperature_tag": "이번 달 분위기 한 단어",
  "category_scores": {{
    "balance":   {{"score": 0, "summary": ""}},
    "operation": {{"score": 0, "summary": ""}},
    "bug":       {{"score": 0, "summary": ""}},
...
## 작성 지침
headline (20자 이내): 이 달 갤러리를 뉴스 헤드라인처럼 압축
summary (3~4문장): 이달 주요 이슈의 원인과 커뮤니티 분위기. 종결어미 '~습니다/ㅂ니다' 체 통일
category_scores: score 0~3 (0=없음, 1=소수 언급, 2=주요 화제, 3=핵심 이슈). 해당 달 전반의 비중으로 판단
major_issues: 이달 가장 많이 논의된 이슈 최대 3개"""
