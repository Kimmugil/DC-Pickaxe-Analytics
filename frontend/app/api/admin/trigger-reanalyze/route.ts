...
export async function POST(req: NextRequest) {
  ...
  const { password, start_date, end_date, gallery_id, dry_run } = body
  ...
  if (gallery_id) inputs.gallery_id = gallery_id

  const res = await fetch(
    `https://api.github.com/repos/${repo}/actions/workflows/backfill_reanalyze.yml/dispatches`,
    {
      method: 'POST',
      headers: {
